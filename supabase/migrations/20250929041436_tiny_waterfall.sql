/*
  ...
  1. New Tables
    ...
    - `collection_products` - Many-to-many relationship for collections
    ...
    - `coupon_uses` - Track coupon usage
    - `returns` - Return requests
    - `inventory_adjustments` - Stock level adjustments
...
BEGIN
  WHILE NOT done LOOP
    new_number := 'SYN-' || to_char(now(), 'YYYYMMDD') || '-' || lpad(floor(random() * 10000)::text, 4, '0');
...
INSERT INTO categories (name, slug, description, is_featured, sort_order) VALUES
  ('Electronics', 'electronics', 'Latest technology and gadgets', true, 1),
  ('Fashion', 'fashion', 'Trendy clothing and accessories', true, 2),
  ('Home & Garden', 'home-garden', 'Everything for your home', true, 3),
  ('Beauty', 'beauty', 'Beauty and personal care products', false, 4),
  ('Sports', 'sports', 'Sports and outdoor equipment', false, 5);

INSERT INTO brands (name, slug, description, is_active) VALUES
  ('TechPro', 'techpro', 'Premium technology brand', true),
  ('StyleMax', 'stylemax', 'Fashion forward clothing', true),
  ('HomeComfort', 'homecomfort', 'Quality home products', true);
...
INSERT INTO products (
  ...
  (
    'TECH-001', 'Premium Wireless Headphones', 'premium-wireless-headphones',
    'High-quality wireless headphones with noise cancellation and premium sound quality.',
    'Premium wireless headphones with noise cancellation',
    25000000, 30000000, 'UGX',
    (SELECT id FROM categories WHERE slug = 'electronics'),
    (SELECT id FROM brands WHERE slug = 'techpro'),
    ARRAY['https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&h=500&fit=crop'],
    'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&h=500&fit=crop',
    15, true, true,
    4.8, 124
  ),
  (
    'TECH-002', 'Smart Fitness Watch', 'smart-fitness-watch',
    'Advanced fitness tracking with heart rate monitoring and GPS.',
    'Smart fitness watch with advanced tracking',
    18000000, NULL, 'UGX',
    (SELECT id FROM categories WHERE slug = 'electronics'),
    (SELECT id FROM brands WHERE slug = 'techpro'),
